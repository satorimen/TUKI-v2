...
export default function MasterCabinet() {
  ...
  if (!data) {
    return <p className="px-5 py-10 text-neutral-500">…</p>;
  }
    ...
    <main className="px-5 pb-10 pt-6">
      ...
      <button
        ...
        className={`mt-5 flex w-full items-center justify-between rounded-2xl px-5 py-4 transition active:scale-[0.98] ${
          master.isActive ? 'bg-green-500/15 text-green-400' : 'bg-[#1c1c1c] text-neutral-400'
        }`}
      >
        ...
        <span
          className={`flex h-7 w-12 items-center rounded-full p-1 transition ${
            master.isActive ? 'justify-end bg-green-500/40' : 'justify-start bg-neutral-700'
          }`}
        >
      ...
      <div className="mt-4 grid grid-cols-3 gap-2.5">
        {[
          ...
          <div key={label} className="rounded-2xl bg-[#1c1c1c] p-3.5 text-center">
      ...
      <div className="mt-4 grid grid-cols-2 gap-2.5">
        <Link
          href="/master/feed"
          className="flex items-center justify-center rounded-2xl bg-tuki-500 py-3.5 text-sm font-semibold text-white transition active:scale-[0.98]"
        >
          📋 {t('feed')}
          {activeBids.length > 0 && (
        ...
        <button
          ...
          className="rounded-2xl border border-neutral-800 py-3.5 text-sm font-medium text-neutral-300 transition active:scale-[0.98]"
        >
          ✏️ {t('edit')}
        </button>
      </div>
      ...
      {/* My bids */}
      <h2 className="mb-3 mt-7 text-sm font-semibold uppercase tracking-wide text-neutral-500">
        {t('activeBids')}
      </h2>
      {data.bids.length === 0 && (
        <p className="rounded-2xl bg-[#1c1c1c] p-5 text-center text-sm text-neutral-500">
          {t('noBids')}
        </p>
      )}
